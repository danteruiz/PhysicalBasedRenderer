...
DebugUI::~DebugUI() { imgui::uninitialize(); }
                   ...
                   std::function<void(std::string)> generateIBLEnvironments)
{
    ...
    if (m_useModel)
    {
        ...
        {
            loadNewModel(gltfModels::getModelPath(m_modelCurrentIndex),
                         m_useModel);
        }
    }
    ...
    if (entity.model)
    {
        ...
        auto &material =
            std::get<0>(entity.model->m_materials[m_materialIndex]);
...
void DebugUI::showStats(float const deltaTime)
{

#define GL_GPU_MEM_INFO_TOTAL_AVAILABLE_MEM_NVX 0x9048
#define GL_GPU_MEM_INFO_CURRENT_AVAILABLE_MEM_NVX 0x9049
    constexpr float KB = 1000000.0f;
    int totalMemory = 0;
    ...
    float usedMemory = (totalMemory - availableMemory) / KB;

    ImGui::Begin("Stats");
    ImGui::Text("DeltaTime: %.6f", deltaTime);
    ImGui::Text("GPU Memory Usage: %.3f GB / %.3f GB", usedMemory,
                static_cast<float>(totalMemory / KB));
    ImGui::End();
}

bool DebugUI::focus() { return m_focus; }
