...
DebugDraw::DebugDraw()
{
    ...
    Attribute positionAttribute(Slot::Position,
                                Format(Type::Float, Dimension::Vec3), 0);
    Attribute colorAttribute(Slot::Color, Format(Type::Float, Dimension::Vec3),
                             positions.size());

    m_attributes = {positionAttribute, colorAttribute};
    // m_debugPipeline = std::make_shared<Shader>(debugFragmentShader,
    // debugVertexShader);
}

void DebugDraw::renderMarkers(std::vector<Marker> const &marker,
                              glm::mat4 const &view,
