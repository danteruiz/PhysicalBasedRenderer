...
static const std::string CONVERT_TO_CUBE_MAP =
    shaderPath + "convertToCubeMap.fs";
static const std::string IRRADIANCE_CONVOLUTION =
    shaderPath + "irradianceConvolution.fs";
...
void rotateCameraAroundEntity(Entity const &entity, float deltaTime)
{
    ...
    camera.orientation =
        glm::quat(glm::radians(glm::vec3(0.0f, yawOffset, 0.0f)));
    camera.position =
        entity.translation + ((camera.orientation * UNIT_Z) * -4.0f);
}
...
DemoApplication::DemoApplication()
{
    ...
    m_modelEntity.rotation =
        glm::quat(glm::radians(glm::vec3(0.0f, 0.0f, 0.0)));
    ...
    m_modelEntity.model->m_materials[0] =
        std::make_tuple(DEFAULT_MATERIAL, m_pipeline);
    m_convertToCubeMap =
        std::make_shared<Shader>(CONVERT_TO_CUBE_MAP, SKYBOX_VERT);
    m_irradiance =
        std::make_shared<Shader>(IRRADIANCE_CONVOLUTION, SKYBOX_VERT);
...
void enableTexture(unsigned int slot, std::shared_ptr<Texture> const &texture)
{
    if (texture)
    {
        enableTexture(slot, texture->m_id);
    }
}
...
void renderModelEntity(RenderArgs const &renderArgs, Backend *backend)
{
    ...
    for (auto mesh : model->m_meshes)
    {
        ...
        for (auto subMesh : mesh.m_subMeshes)
        {
            ...
            glDrawElements(GL_TRIANGLES, (GLsizei)subMesh.m_numIndices,
                           GL_UNSIGNED_INT,
...
std::vector<Marker> getMarkers(RenderArgs const &renderArgs)
{
    ...
    auto entity = renderArgs.modelEntity;
    Marker entityMarker;
    entityMarker.m_matrix =
        getMatrix(entity, false) * entity.model->m_meshes[0].m_matrix;
...
void drawSkybox(Skybox const &skybox, RenderArgs const &renderArgs,
                Backend *backend)
{
    ...
    glBindTexture(GL_TEXTURE_CUBE_MAP,
                  renderArgs.useIrradianceMap ? irradianceMap : envCubemap);
...
void DemoApplication::exec()
{
    ...
    while (!m_window->shouldClose())
    {
        ...
        glm::mat4 projection = glm::perspective(
            glm::radians(60.0f),
            (float)m_window->getWidth() / (float)m_window->getHeight(), 0.3f,
            700.0f);
        ...
        auto loadNewModel = [&](std::string path, bool useModel)
        {
            ...
            {
                m_modelEntity.model =
                    m_modelCache->getModelShape(ModelShape::Sphere);
            }
        };
        ...
        m_debugUI->show(m_modelEntity, m_light, deltaTime, loadNewModel,
                        genEnv);
...
constexpr int32_t SKYBOX_RESOLUTION = 4096;
unsigned int DemoApplication::generateEnviromentMap()
{
    ...
    for (unsigned int i = 0; i < 6; ++i)
    {
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                               GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, textureId,
                               0);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
...
void DemoApplication::generateIBLEnvironment(std::string &texturePath)
{
    ...
    glViewport(0, 0, 32, 32); // don't forget to configure the viewport to the
                              // capture dimensions.
    glBindFramebuffer(GL_FRAMEBUFFER, captureFBO);
    for (unsigned int i = 0; i < 6; ++i)
    {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RG16F, 1080,
                 1080, 0, GL_RG, GL_FLOAT, 0);
